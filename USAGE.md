...
```bash
export SKILL_DIR="$PWD/.trae/skills/akshare-fund-advisor"
...
| --- | --- |
| `--years` | `3`、`5`、`10`、`20` |
| `--source` | `auto`、`wind`、`akshare` |
| `--max-points` | `50` 到 `3000` |

指数名称必须精确。支持的别名和代码由脚本 `INDEX_ALIASES`、`INDEX_CATALOG` 定义；“沪深300非银行金融”不会模糊映射成“沪深300”。
...
WindPy 由用户自己的 Wind 安装提供，不在 `requirements.txt` 中。需要：

1. Wind 终端已安装并登录。
2. 当前 Python 环境可导入 `WindPy`。
3. 账号拥有目标指数和 `PE_TTM,PB_LF` 权限。

`--source wind` 失败时直接返回错误。`--source auto` 对有 AKShare 映射的指数会记录 Wind 失败后回退。
...
AKShare 兼容视图不是 Wind 数据。必须按 `actual_source.provider` 标注来源。
...
使用精确指数名称、已支持别名或 Wind 代码。不要依赖子串匹配。
...
该指数没有 AKShare 回退，且 WindPy 不可用、连接失败或无权限。
