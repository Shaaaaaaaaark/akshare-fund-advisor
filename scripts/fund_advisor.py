...
WIND_API_DOC = "https://www.wind.com.cn/mobile/ClientApi/zh.html"
...
INDEX_CATALOG = {
    "创业板50": {"akshare_symbol": "创业板50", "index_code": "399673", "wind_code": "399673.SZ"},
    "中证1000": {"akshare_symbol": "中证1000", "index_code": "000852", "wind_code": "000852.SH"},
    "沪深300": {"akshare_symbol": "沪深300", "index_code": "000300", "wind_code": "000300.SH"},
    "上证380": {"akshare_symbol": "上证380", "index_code": "000009", "wind_code": "000009.SH"},
    "中证500": {"akshare_symbol": "中证500", "index_code": "000905", "wind_code": "000905.SH"},
    "上证180": {"akshare_symbol": "上证180", "index_code": "000010", "wind_code": "000010.SH"},
    "深证红利": {"akshare_symbol": "深证红利", "index_code": "399324", "wind_code": "399324.SZ"},
    "深证100": {"akshare_symbol": "深证100", "index_code": "399330", "wind_code": "399330.SZ"},
    "上证红利": {"akshare_symbol": "上证红利", "index_code": "000015", "wind_code": "000015.SH"},
    "中证100": {"akshare_symbol": "中证100", "index_code": "000903", "wind_code": "000903.SH"},
    "中证800": {"akshare_symbol": "中证800", "index_code": "000906", "wind_code": "000906.SH"},
    "上证50": {"akshare_symbol": "上证50", "index_code": "000016", "wind_code": "000016.SH"},
    "万得全A": {"akshare_symbol": None, "index_code": "881001", "wind_code": "881001.WI"},
}
INDEX_ALIASES = {
    "红利指数": "上证红利",
    "上证红利指数": "上证红利",
...
class FundAdvisor:
    ...
    def _resolve_index(query: str) -> Dict[str, Any]:
        ...
        query_without_suffix = query_upper.split(".", 1)[0]
        for name, spec in INDEX_CATALOG.items():
            if (
                query_upper == str(spec["wind_code"]).upper()
                or query_without_suffix == str(spec["index_code"]).upper()
            ):
                return {"name": name, **spec}

        if re.fullmatch(r"[A-Z0-9]+\.(SH|SZ|WI|CSI|HI)", query_upper):
            return {
                "name": query_upper,
                "akshare_symbol": None,
                "index_code": query_without_suffix,
                "wind_code": query_upper,
            }
        raise AdvisorError(
            "INDEX_NOT_SUPPORTED",
            f"暂未找到可用估值映射：{query}",
            {
                ...
                "guidance": (
                    "请传入精确指数名称、已列出的别名或 Wind 代码；"
                    "其他指数可传 Wind 代码并使用 --source wind。"
                ),
            },
        )

    def _load_wind_valuation_frames(
        self,
        index: Dict[str, Any],
        years: int,
    ) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, Any]]:
        if importlib.util.find_spec("WindPy") is None:
            raise AdvisorError(
                "WINDPY_UNAVAILABLE",
                "当前环境未安装 WindPy，无法调用 Wind Client API",
            )
        from WindPy import w

        start_date = (self.now.date() - timedelta(days=years * 366)).isoformat()
        end_date = self.now.date().isoformat()
        with deadline(self.timeout_seconds):
            start_result = w.start(waitTime=self.timeout_seconds)
            start_error = getattr(start_result, "ErrorCode", 0)
            if start_error not in (0, None) or not w.isconnected():
                raise AdvisorError(
                    "WIND_CONNECTION_ERROR",
                    "Wind Client API 未连接或账号无权限",
        ...
        if getattr(data, "ErrorCode", -1) != 0:
            raise AdvisorError(
                ...
                "Wind WSD 指数估值查询失败",
                {
                    "error_code": getattr(data, "ErrorCode", None),
                    "wind_code": index["wind_code"],
                },
            )

        field_data = {
            str(field).upper(): values
            for field, values in zip(data.Fields, data.Data)
        }
        missing_fields = [
            field for field in ("PE_TTM", "PB_LF") if field not in field_data
        ]
        if missing_fields:
            raise AdvisorError(
                "WIND_FIELD_ERROR",
        ...
        dates = pd.to_datetime(data.Times, errors="coerce")
        pe_frame = pd.DataFrame(
            {
                "日期": dates,
                "滚动市盈率": field_data["PE_TTM"],
            }
        )
        pb_frame = pd.DataFrame(
            {
                "日期": dates,
                "市净率": field_data["PB_LF"],
            }
        )
        self._add_source(
            "w.wsd",
        ...
        combined_frame = pd.DataFrame(
            {
                "日期": dates,
                "PE_TTM": field_data["PE_TTM"],
                "PB_LF": field_data["PB_LF"],
            }
        )
        self.data_audit.append(
            {
                "interface": "w.wsd",
                "parameters": {
                    "code": index["wind_code"],
                    "fields": ["PE_TTM", "PB_LF"],
                    "start_date": start_date,
                    "end_date": end_date,
                    "options": "Period=D;Days=Trading",
                },
                "row_count": len(combined_frame),
                "columns": ["日期", "PE_TTM", "PB_LF"],
                "frame_sha256": frame_fingerprint(combined_frame),
                "validation": "passed",
                "skill_transform_at_ingestion": "none",
                "received_from_provider": "Wind WSD response converted to DataFrame",
            }
        )
        return (
            pe_frame,
    ...
    def _load_akshare_valuation_frames(
        self,
        index: Dict[str, Any],
    ) -> Tuple[
        Optional[pd.DataFrame],
        ...
        return (
            ...
            pb_frame,
            {
                ...
                "upstream": "乐咕乐股",
                "fields": {
                    "PE_TTM_compatible": "滚动市盈率",
                    "PB_LF_compatible": "市净率",
                },
                "available_metrics": [
                    metric
                    for metric, frame in (("PE_TTM", pe_frame), ("PB_LF", pb_frame))
                    if frame is not None
                ],
                "licensed_data": False,
                "note": "字段口径参考 Wind PE_TTM/PB_LF，但数据源不是 Wind。",
            },
        )
    ...
    def _load_valuation_frames(
        ...
        wind_error: Optional[AdvisorError] = None
        if source in {"auto", "wind"}:
            wind_available = importlib.util.find_spec("WindPy") is not None
            if source == "wind" or wind_available:
                try:
                    return self._load_wind_valuation_frames(index, years)
                except Exception as exc:
                    wind_failure = (
                        exc
                        if isinstance(exc, AdvisorError)
                            ...
                            "WIND_SOURCE_ERROR",
                            "Wind Client API 查询失败",
                            {
                                "wind_code": index.get("wind_code"),
                                "reason": str(exc),
                            },
                        )
                    )
                    self.data_audit.append(
                        {
                            "interface": "w.wsd",
                            "parameters": {
                                "code": index.get("wind_code"),
                                "fields": ["PE_TTM", "PB_LF"],
                                "lookback_years": years,
                            },
                            "validation": "failed",
                            "error": {
                                "code": wind_failure.code,
                                "message": wind_failure.message,
                                "details": wind_failure.details,
                            },
                        }
                    )
                    if source == "wind":
                        if isinstance(exc, AdvisorError):
                            raise
                        raise wind_failure from exc
                    wind_error = wind_failure
                    self._warn("w.wsd", wind_failure)
            if source == "auto" and not index.get("akshare_symbol"):
                raise AdvisorError(
                    "VALUATION_SOURCE_UNAVAILABLE",
                    (
                        f"{index['name']} 没有 AKShare 同口径回退数据；"
                        "需要安装 WindPy、连接 Wind 终端并具备指数估值权限"
                    ),
                    {
                        "windpy_installed": wind_available,
                        "wind_error": (
                            wind_error.message if wind_error is not None else None
                        ),
                        "wind_code": index.get("wind_code"),
                    },
                )
        return self._load_akshare_valuation_frames(index)
    ...
    def valuation(
        ...
        pe_frame, pb_frame, source_info = self._load_valuation_frames(
            index,
            years,
            source,
        )
        ...
        for metric_name, metric in (("PE_TTM", pe), ("PB_LF", pb)):
            if metric is None:
                continue
        ...
        start_dates = [
            metric["actual_start_date"] for metric in (pe, pb) if metric is not None
        ]
        latest_dates = [
            metric["latest_date"] for metric in (pe, pb) if metric is not None
        ]
        return {
            "ok": True,
            "action": "valuation",
            ...
            "wind_api_reference": {
                "method": "w.wsd",
                "template": (
                    f'w.wsd("{index["wind_code"]}", "PE_TTM,PB_LF", '
                    '"开始日期", "结束日期", '
                    '"Period=D;Days=Trading")'
                ),
                "note": (
                    "只有 actual_source.provider=Wind 时结果才来自 Wind；"
                    "否则只是按 Wind 字段语义构造的兼容视图。"
                ),
            },
            "limitations": [
                "不同数据商对亏损成分、权重和异常值处理可能不同，数值不保证与 Wind 手机端逐点一致。",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    valuation_parser = subparsers.add_parser(
        "valuation",
        help="生成 Wind 风格指数历史 PE/PB 图表数据",
    )
    valuation_parser.add_argument(
        "--index",
        required=True,
        help="指数名称、常用指数代码或 Wind 代码",
    )
    valuation_parser.add_argument(
        "--years",
        type=int,
    ...
    valuation_parser.add_argument(
        ...
        default="auto",
        help="auto 优先使用本机可用的 WindPy，否则回退 AKShare",
    )
    valuation_parser.add_argument(
        "--max-points",
