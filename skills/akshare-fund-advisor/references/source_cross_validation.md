...
| 数据源 | 审计版本 | 包元数据 | 接入结论 |
| --- | --- | --- | --- |
| Baostock | `0.9.3` | BSD License | 锁定为默认安装、默认关闭的可选 A 股校验源 |
...
审计窗口：`2026-07-18` 至 `2026-08-17`。最新有效交易日均为 `2026-08-14`。

| 数据源 | 类别 | 代表标的 | 结果 |
| --- | --- | --- | --- |
| Baostock | A 股 | `600519`、`000001`、`300750` | `3/3` 通过 |
| Baostock | ETF | `510300`、`159915` | `2/2` 通过 |
| Baostock | 指数 | 沪深 300、中证 500、创业板 50 | `3/3` 通过 |
...
| 标的 | `frame_sha256` |
| --- | --- |
| Baostock `600519` | `916d6e8e6be6e024ca8cbdd5fee1a66d0968f46d062aaf55ee5a4b2ff2141432` |
| Baostock 沪深 300 | `e08c489542bb12bc06ef734ce56e8a651e76eefbb1e1722828935950f28bdcf8` |
...
当前只在 `stock_valuation` 中接入 A 股收盘价校验：

1. 主图继续使用 AKShare `stock_zh_a_daily(adjust="qfq")` 前复权价格。
2. 交叉校验另取 AKShare 与 Baostock/efinance 的不复权日线。
3. 日期按内连接比较，不插值、不前向填充。
4. 相对容忍度为 `0.001`，绝对容忍度为 `0.01`；任一容忍范围内均视为一致。
5. 超阈值差异只产生 `SOURCE_DISAGREE`，主图和主源事实保持不变。
...
默认不设置 `AKSHARE_FUND_SOURCE_VALIDATION`，因此不会增加外部校验调用。

## 5. 暂不接入

- efinance 股票和 ETF 行情本次全部失败，且许可声明冲突，不进入默认生产路径。
- efinance 基金净值虽通过审计，但尚未接入生产 Comparator。
- Baostock ETF 和指数仅完成接口与代码映射审计，尚未接入生产 Comparator。
