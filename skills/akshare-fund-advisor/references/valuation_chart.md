...
本功能使用 AKShare 指数估值接口，参考 Wind 手机端“深度资料 → 指数估值 → 历史
PE/PB”的信息组织方式，但不复制其品牌、专有界面或受限数据。目标不是只显示当前值，
而是把当前估值放入可核验的历史曲线中：
...
- 不使用 Wind 商标，不声称是 Wind 数据或 Wind 原版界面。
...
- 估值看板列表可以展示当前 PE/PB 和历史分位，但指数详情必须展示历史双图。
- 默认使用十年或实际可用最长区间，并允许切换响应中真实存在的 3/5/10/20 年窗口。
- PE 与 PB 上下排列并共享时间范围、十字准星和缩放状态。
- 前端只能连接 `chart_series` 返回的真实点，不得线性插值、前向填充或模型补点。
- 统计卡和参考线必须读取工具返回字段，前端不得重复计算历史分位。
- `source_observations` 表示完整统计样本，`displayed_points` 表示图表抽样点，二者必须分别
  展示或在口径说明中披露。
