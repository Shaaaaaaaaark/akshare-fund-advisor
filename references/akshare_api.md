...
指数历史估值图另见 [valuation_chart.md](valuation_chart.md)。该功能可选调用 Wind Client API `w.wsd`；WindPy 不属于 Skill 依赖，需要用户自己的终端、账号和权限。
...
| `charts.pe_ttm.chart_series` | Wind `PE_TTM` 语义的历史序列 | 来源可能是 Wind 或 AKShare，检查 `actual_source` |
| `charts.pb_lf.chart_series` | Wind `PB_LF` 语义的历史序列 | 来源可能是 Wind 或 AKShare，检查 `actual_source` |
...
Wind 成功审计使用相同的 `skill_transform_at_ingestion` 字段，并将 `received_from_provider` 标记为由 Wind WSD 响应转换的 DataFrame。`frame_sha256` 是规范化 DataFrame 内容指纹，不是 HTTP 原始响应字节哈希。
...
- 不插值、不前向填充；Wind WSD 也不设置 `Fill=Previous`。
