"""MCP tool clients used by LangGraph nodes."""
...
class InProcessFundToolClient:
    """Fast local path that uses the exact same adapter as the MCP server."""
...
class StdioFundToolClient:
    """Protocol path for testing and isolated local deployment."""
    ...
    def _command(self) -> tuple[str, list[str]]:
        ...
        return sys.executable, ["-m", "financial_agent.mcp_server.server"]
...
class HTTPFundToolClient:
    """Streamable HTTP MCP client for the isolated Compose service."""
    ...
    async def call(self, tool: str, arguments: dict[str, Any]) -> ToolEnvelope:
        ...
        async with streamable_http_client(self._config.mcp.server_url) as (reader, writer, _):
            async with ClientSession(reader, writer) as session:
                await session.initialize()
                result = await session.call_tool(tool, arguments=arguments)
        return _parse_result(tool, result)
...
def _parse_result(tool: str, result: Any) -> ToolEnvelope:
    ...
    for item in content:
        ...
        try:
            ...
            raise RuntimeError(
                f"MCP 工具 {tool} 返回了非 JSON 文本：{text[:200]}"
            ) from exc
        candidate = parsed.get("result", parsed) if isinstance(parsed, dict) else parsed
        return ToolEnvelope.model_validate(candidate)
