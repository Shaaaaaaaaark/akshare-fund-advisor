...
| --- | --- |
| Agent Skill 包装 | 已实现 |
| `data_core` | 已建立，`fund_search`、`fund_status` 已迁移；ETF 最近份额、ETF/成分融资补充指标已接入 |
| Data API | 已实现 |
...
3. 已对精确映射到支持指数的 ETF 接入最新成份快照与沪深融资明细汇总；
4. 后续补充可直接审计的净申赎、融资长期分位和机构持仓来源，不用估算值冒充源事实；
5. 继续同步 Data API、`ToolEnvelope`、时效、错误语义和真实接口审计。
